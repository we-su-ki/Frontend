...
const AXIS_KEYS = [
  'sweetness', 'sourness', 'bitterness', 'umamiSalty',
  'fruity', 'citrus', 'floral', 'herbal',
  'spicy', 'woodySmoky', 'body', 'fizzy',
];
...
export default function TasteRadarChart({ tasteProfile = {}, size = 260 }) {
  ...
  const dataPoints = useMemo(() => {
    return AXIS_KEYS.map((key, i) => {
      const val = Math.min(Math.max(tasteProfile[key] ?? 0, 0), MAX_VAL);
  ...
  return (
    ...
    >
      ...
      {/* Axis labels — now translated via tFlavor() */}
      {AXIS_KEYS.map((key, i) => {
        const angle = (360 / N) * i;
